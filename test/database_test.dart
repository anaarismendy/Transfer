...
void main() {
  ...
  Future<User> createUser(String name, String email) async {
    final result = await users.create(name: name, email: email, passwordHash: 'hash');
  ...
  group('usuarios', () {
    ...
    test('editar un usuario que no existe da NotFound', () async {
      const ghost = User(id: 'nope', name: 'X', email: 'x@x.co', passwordHash: 'h');
  ...
  group('transferencias', () {
    test('se registra y aparece en el historial', () async {
      ...
      final first = all.fold((f) => throw StateError(f.message), (t) => t.single);

      expect(first.amountInCents, 150000);
      expect(first.description, 'Arriendo', reason: 'debe venir sin espacios sobrantes');
    });

    test('la descripcion vacia se guarda como nula, no como cadena vacia', () async {
      final ana = await createUser('Ana', 'ana@test.com');
      final luis = await createUser('Luis', 'luis@test.com');
    ...
    test('el CHECK de la base rechaza un valor de cero o negativo', () async {
      ...
      expect(await transfers.getAll().then((r) => r.fold((_) => -1, (t) => t.length)), 0);
    });
    ...
    test('el historial viene con la mas reciente primero', () async {
      ...
      await transfers.create(sourceUserId: ana.id, destinationUserId: luis.id, amountInCents: 100);
      await transfers.create(sourceUserId: ana.id, destinationUserId: luis.id, amountInCents: 200);
    ...
    test('no se puede borrar un usuario con movimientos', () async {
      ...
      expect(failureOf(await users.delete(ana.id)), isA<UserHasTransfersFailure>());
    });
  });

  group('sesion', () {
    test('guarda, lee y limpia', () async {
      expect(await auth.currentSessionUserId().then((r) => r.fold((_) => null, (id) => id)), isNull);

      await auth.saveSession('user-1');
      expect(
        await auth.currentSessionUserId().then((r) => r.fold((_) => null, (id) => id)),
        'user-1',
      );
